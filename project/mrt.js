...
mrt.get_fp = function (alt, az, posture) {
  ...
  if (posture === "standing" || posture === "supine") {
    var fp_table = [
      ...
      /*azm=15*/ [0.343, 0.343, 0.312, 0.254, 0.2, 0.14, 0.082],
      /*azm=30*/ [0.332, 0.332, 0.3, 0.242, 0.19, 0.134, 0.082],
      /*azm=45*/ [0.313, 0.313, 0.278, 0.227, 0.176, 0.123, 0.082],
      /*azm=60*/ [0.283, 0.283, 0.25, 0.207, 0.161, 0.113, 0.082],
      /*azm=75*/ [0.253, 0.253, 0.228, 0.188, 0.15, 0.107, 0.082],
      /*azm=90*/ [0.23, 0.23, 0.213, 0.18, 0.148, 0.108, 0.082],
      /*azm=105*/ [0.242, 0.242, 0.222, 0.188, 0.154, 0.111, 0.082],
      /*azm=120*/ [0.274, 0.274, 0.245, 0.204, 0.164, 0.115, 0.082],
      /*azm=135*/ [0.303, 0.303, 0.27, 0.22, 0.174, 0.121, 0.082],
    ...
    var fp_table = [
      /*azm=0*/ [0.29, 0.325, 0.305, 0.305, 0.262, 0.223, 0.178],
      /*azm=15*/ [0.293, 0.328, 0.294, 0.288, 0.268, 0.225, 0.178],
      /*azm=30*/ [0.288, 0.333, 0.298, 0.29, 0.262, 0.222, 0.178],
      ...
      /*azm=90*/ [0.215, 0.26, 0.242, 0.248, 0.22, 0.186, 0.178],
      /*azm=105*/ [0.235, 0.258, 0.228, 0.237, 0.209, 0.18, 0.178],
      /*azm=120*/ [0.263, 0.259, 0.208, 0.223, 0.196, 0.175, 0.178],
      /*azm=135*/ [0.28, 0.26, 0.192, 0.21, 0.185, 0.17, 0.178],
      /*azm=150*/ [0.3, 0.258, 0.175, 0.195, 0.169, 0.168, 0.178],
      /*azm=165*/ [0.305, 0.25, 0.158, 0.18, 0.155, 0.164, 0.178],
      /*azm=180*/ [0.3, 0.24, 0.152, 0.169, 0.154, 0.165, 0.178],
    ];
  }
...
mrt.fanger_view_factor = function (
  surface_pos,
  d_perp,
  area,
  person_pos,
  ...
  var view_factor =
    (((projected_area_factor / Math.PI) * d_perp) / Math.pow(dist, 3)) * area;
  return view_factor;
};
  ...
  for (var i = 0; i < mesh.length; i++) {
    ...
    if (average_azimuths) {
      ...
      for (var iaz = 0; iaz < az_L; iaz++) {
        sum_view_factor += mrt.fanger_view_factor(
          p,
          d_perp,
          area,
          person_pos,
      ...
      var dvf = mrt.fanger_view_factor(
        p,
        d_perp,
        area,
        person_pos,
...
mrt.view_factors = function () {
  ...
  for (var i = 0; i < mrt.walls.length; i++) {
    ...
    var d_perp = Math.abs(
      mrt.occupant.position[mesh.perpendicular_plane] -
        surface.offset[mesh.perpendicular_plane]
    );
    var vf = mrt.mesh_view_factor(
      mesh.mesh,
      d_perp,
      mesh_area,
      mrt.occupant.position,
      mrt.occupant.azimuth,
    ...
    if (surface.subsurface && surface.subsurfaces.length > 0) {
      for (var j = 0; j < surface.subsurfaces.length; j++) {
        ...
        // optimize by not recomputing this mesh every time
        var subsurface_mesh = mrt.create_mesh(
          subsurface.width,
          subsurface.height,
          offset,
          surface.plane,
        ...
        var subsurface_mesh_area =
          ((subsurface.width / Nu) * subsurface.height) / Nv;

        var sub_vf = mrt.mesh_view_factor(
          subsurface_mesh.mesh,
          d_perp,
          subsurface_mesh_area,
          mrt.occupant.position,
          mrt.occupant.azimuth,
          mrt.occupant.posture,
          average_azimuths
...
mrt.calc = function (surface_vfs) {
  ...
  for (var i = 0; i < surface_vfs.length; i++) {
    ...
    if (s.hasOwnProperty("parent")) {
      var parent = _.find(mrt.walls, function (w) {
        return w.name === s.parent;
      });
      var surface = _.find(parent.subsurfaces, function (x) {
    ...
    my_mrt +=
      vf * surface.emissivity * Math.pow(surface.temperature + 273.15, 4);
  }
...
mrt.test = function () {
  ...
  var d_perp =
    person_pos[mesh.perpendicular_plane] - offset[mesh.perpendicular_plane];
