...
function init(canva) {
  ...
  camera = new THREE.PerspectiveCamera(1, window.innerWidth / window.innerHeight, 10, 1000);
...
function initTweakPane() {
  ...
  const climate_pane = global_pane.addFolder({
  ...
  const time_pane = global_pane.addFolder({
  ...
  const geometry_pane = global_pane.addFolder({
  ...
  const mrt_pane = global_pane.addFolder({
...
geometry = new THREE.CircleGeometry( 0.83, 32 );
  material = new THREE.MeshBasicMaterial( { color: 0x333333 } );
  circle = new THREE.Mesh( geometry, material );
  circle.position.x = 6
      circle.position.y = 3.5
      circle.position.z = -1.01
scene.add( circle );
  ...
  var arrowHelper = new THREE.ArrowHelper(dir, origin, length, 0x333333 , 0.3, 0.3);
  arrowHelper.scale.z = 0.1
